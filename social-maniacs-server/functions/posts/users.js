    ...
    db.doc(`/users/${newUser.name}`).get()
        ...
        .catch(err => {
            ...
            if(err.code === "auth/email-already-in-use"){
                return res.status(400).json({ email: 'Email already in use, be original!'});
            } else {
...
exports.getUserDetails = (req, res) => {
    ...
    .then(doc => {
        if(doc.exists){
            ...
            return db.collection('screams').where('userName', '==', req.params.name)
    ...
    db.doc(`/users/${req.user.name}`).get()
        .then(doc => {
            if(doc.exists){
              ...
              return db.collection('likes').where('userName', '==', req.user.name).get() 
            }
        })
        .then((data) => {
            userData.likes = [];
